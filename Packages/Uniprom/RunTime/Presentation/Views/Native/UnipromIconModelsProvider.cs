...
namespace Uniprom
{
    public abstract class UnipromIconModelsProvider : UnipromModelsProvider<UnipromIconModel>
    {
        readonly Queue<string> _excludeAppKeys = new ();
        
        protected override Task<List<UnipromIconModel>> LoadModelsAsync(CancellationToken token, int length)
        {
            if (HasModel)
            {
                SetUpExcludeAppKeys(length);
            }
            return UnipromManager.Instance.LoadIconModelsAsync(token, length, _excludeAppKeys);
        }
